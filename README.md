...
카카오 로그인 API를 이용하여 카카오 계정으로 로그인하거나 자체 회원가입을 하여 로그인 할 수 있다.
...
거래량 TOP10 코인 목록을 확인할 수 있으며, 이를 클릭하여 매수/매도할 수 있다. 
...
유저들의 총 자산이 실시간으로 업데이트 되어 자신의 보유 자산 순위를 알 수 있다. 
...
* 회원탈퇴 - DB에 저장된 유저 정보가 삭제되어 앱을 탈퇴할 수 있다.
* 비밀번호 변경 - 저장된 비밀번호를 변경할 수 있다. 
* 로그아웃 - 앱 로그아웃을 할 수 있다. 
